...
for (iter in 1:simul)
{
  ...
  #randtvec<-matrix(rt(objects*times,df=mydf),nrow=objects,ncol=times);
  ...
  trueclust<-c(seq(from=1,to=1,length=clussizes[1]), seq(from=2,to=2,length=(clussizes[2]-clussizes[1])), seq(from=3,to=3,length=(clussizes[3]-clussizes[2])), seq(from=4,to=4,length=(clussizes[4]-clussizes[3])))
  ...
  #################################################################
  # #
  # loop to create approximation of normal errors 0, std=0.3: 

  sigma_e <- 0.3  
  ouvalue <- matrix(
    rnorm(objects * times, mean = 0, sd = sigma_e),
    nrow = objects,
    ncol = times
  )
  #################################################################
  ...
  #################################################################
  # #
  #        Simulations of Signal curves #
  # These are the "true" underlying curves #
  
  curves<-matrix(0:0,nrow=objects,ncol=times)
  ...
  #{
  #curves[rowi,]<-0.6*cos(timevec)*log(timevec+0.5)*sqrt(timevec+0.5)
  #}
  ...
  #for (rowi in (clussizes[3]+1):clussizes[4])
  #{
  #curves[rowi,]<-1.4*cos(timevec)*log(2*timevec+0.5)*sqrt(timevec+0.5)
  #}
  ...
  rand<-c()
  bestrand<-c()
  btgroup<-c(rep(1,5),rep(2,5),rep(3,5),rep(4,5))
...
write.csv(randtot,"C:/research/2026/randtotbsplinebasisM3normalerror.csv",row.names = FALSE)
...
# Plotting one observed curve from each cluster:
plot(timevec, obscurves[clussizes[1],], type='l', ylim=c(-6,3),col="red")
lines(timevec, obscurves[clussizes[2],], lty=2,col="green")
lines(timevec, obscurves[clussizes[3],], lty=3,col="blue")
lines(timevec, obscurves[clussizes[4],], lty=4)
#
