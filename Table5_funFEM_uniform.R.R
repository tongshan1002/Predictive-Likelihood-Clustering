...
# target variance
tau2 <- 0.125
theta <- sqrt(3 * tau2)   # so Var = theta^2/3 = tau2
...
for (iter in 1:simul) {
  ...
  # -----------------------
  # Uniform noise: Unif(-theta, theta), Var = tau2
  eps <- matrix(runif(objects * times, min = -theta, max = theta),
...
cat("Uniform noise: tau^2 =", tau2, " | theta =", theta, "\n")
...
cat("funFEM | K=4 | n=20 (5 each) | Uniform(-theta,theta), Var=tau^2\n")
