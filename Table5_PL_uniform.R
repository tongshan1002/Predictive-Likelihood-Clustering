...
for (iter in 1:simul)
{
  ...
  #################################################################
  # #
  # loop to create approximation of Uniform errors: Unif(-0.6, 0.6)

  ouvalue <- matrix(
    runif(objects * times, min = -0.6, max = 0.6),
    nrow = objects,
    ncol = times
  )
...
write.csv(randtot,"C:/research/2026/randtotbsplinebasisM3uniferror.csv",row.names = FALSE)
