...
VDPdata <- read.table("C:/research/VDPdata.txt", header=T)


VDPdata <- cbind(timevec,tranobscurves)
btgroup<-c(rep(1,9),rep(2,11),rep(3,4))
...
T4=matrix(cos(2*pi*x1),ncol=1)
T5=matrix(sin(2*pi*x1),ncol=1)
T6=matrix(cos(4*pi*x1),ncol=1)
T7=matrix(sin(4*pi*x1),ncol=1)
T=cbind(T1,T2,T3,T4,T5,T6,T7)
...
predictlikelihood<-function(gr){
  ...
  for (i in 1:length(unique(gr))){
    for (j in 1:m[i]){
      linear.model<-lm(u[[i]][,j]~x1+I(x1^2)+I(cos(2*pi*x1))+I(sin(2*pi*x1))+I(cos(4*pi*x1))+I(sin(4*pi*x1)))
...
predictlikelihoodsilhouette<-function(gr){
  ...
  newlist <- list(oldc,beta0,beta1,beta2,beta3,beta4,beta5,beta6)
...
for (Tm in 1:10000){
  ...
  if (count.odd==1){
    ...
    count.odd<-abs(1-count.odd)}
      ...
      matrix.of.betahats<-cbind(beta0,beta1,beta2,beta3,beta4,beta5,beta6)
